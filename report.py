...
class Report:
    ...
    def generate_building_report_beta(self, report_path):
        ...
        with open(report_file, 'w', encoding="utf-8") as report_html:
            ...
            report_html.write('<title>Building Efficiency Targeting Tool for Energy Retrofits (BETTER) Reporte</title>')
            ...
            report_html.write('    <td class="td_border" colspan="3"><b>Tipo de construcción</b></td>')
            report_html.write('    <td class="td_border" colspan="3">' + self.building.bldg_type + '</td>')
            report_html.write('    <td class="td_border" colspan="3"><b>Ubicación del edificio</b></td>')
            report_html.write('    <td class="td_border" colspan="3">' + self.building.bldg_address + '</td>')
            report_html.write('  </tr>')
            report_html.write('  <tr>')
            report_html.write('    <td class="td_border" colspan="3"><b>Superficie de suelo (m<sup>2</sup>)</b></td>')
            ...
            report_html.write('    <td class="td_border" colspan="4"><b>Electricidad</b></td>')
            report_html.write('    <td class="td_border" colspan="4"><b>Combustible fósil</b></td>')
            report_html.write('  </tr>')
            report_html.write('  <tr>')
            report_html.write('    <td class="td_border" colspan="4"><b>Consumo anual (kWh)</b></td>')
            if(hasattr(self.building, 'recent_annual_electricity_kWh')):
                ...
                report_html.write('    <td class="td_border" colspan="4">No hay datos de electricidad.</td>')
          #  report_html.write('    <td class="td_border" colspan="4"><b>Annual Fossil Fuel Consumption (kWh)</b></td>')
            if(hasattr(self.building, 'recent_annual_fossil_fuel_kWh')):
                ...
                report_html.write('    <td class="td_border" colspan="4">No hay datos de combustibles fósiles.</td>')
            report_html.write('  </tr>')
            report_html.write('  <tr>')
            report_html.write('    <td class="td_border" colspan="4"><b>Costo anual (' + self.currency_str + ')</b></td>')
          ...
          #  report_html.write('    <td class="td_border" colspan="4"><b>Annual Site EUI (kWh/m<sup>2</sup>)</b></td>')
            ...
            report_html.write('  </tr>')
            report_html.write('    </table>')
            report_html.write('<p style="margin:0px;">&nbsp;&nbsp;<i>Nota: Los resultados anuales son de los últimos 12 meses ingresados.</i></p>')
            ...
            report_html.write('<br>')
            report_html.write('  <div class="w3-container w3-padding-large w3-white">')
            report_html.write('    <h2 id="about"><b>Potenciales de ahorro</b></h2>')
            report_html.write('    <hr class="w3-opacity">')
            ...
            report_html.write('<p class="w3-xlarge">    Posibles ahorros de costos: </p><p class="number">' + self.currency_str + " {:,}".format(
                int(self.building.total_cost_savings)) + '</p>')
            report_html.write('<p class="w3-xlarge">    Porcentaje potencial de ahorro: </p><p class = "number">' + "{:,}".format(
            ...
            report_html.write('<h5><b>Recomendaciones de eficiencia energética</b></h5>')
            ...
            report_html.write('<hr><button class="w3-button w3-border w3-hover-grey" onclick="showTrends()">Mostrar / Ocultar consumo original y previsto con actualización</button>')
            report_html.write('</div>')
            ...
            if(hasattr(self.building, 'energy_savings_fig_e')):
                ...
                report_html.write('<p>No hay datos de electricidad.</p>')
            if(hasattr(self.building, 'energy_savings_fig_f')):
                report_html.write(self.building.energy_savings_fig_f)
            else:
                report_html.write('<p>No hay datos de combustibles fósiles.</p>')
            report_html.write('</div>')
            report_html.write('</div>')
            # Savings plots end
            report_html.write('</div>')
            ...
            report_html.write('    <h2 id="benchmark"><b>Sensibilidad al clima y puntos de referencia</b></h2>')
            report_html.write('    <hr class="w3-opacity">')
            report_html.write(
                "El consumo diario de electricidad y combustible fósil por área de piso se representa a continuación en comparación con la temperatura promedio mensual del aire exterior. Cuando el uso de energía aumenta a bajas temperaturas en el lado izquierdo del gráfico, representa la energía de calentamiento. Cuando el uso de energía aumenta a altas temperaturas en el lado derecho del gráfico, representa la energía de enfriamiento. La parte plana del gráfico muestra la carga base del edificio.")
            ...
            report_html.write('    <div class="w3-container ">')
            report_html.write('      <p><b>Modelo de punto de cambio de electricidad</b></p>')
            if (hasattr(self.building, 'im_electricity') and hasattr(self.building.im_electricity, 'model_description_html')):
                ...
                report_html.write(
                    '<p>No se proporcionan datos de consumo de electricidad o no se encontró un modelo de punto de cambio significativo para la electricidad.</p>')
            report_html.write('    </div>')
            report_html.write('  </div>')
            ...
            report_html.write('<p><i>Nota:% indica el porcentaje de edificios a los que su edificio es superior.</i></p>')
            report_html.write(
            ...
            report_html.write('  <div class="w3-half w3-container w3-margin-bottom w3-col m5 w3-padding-small">')
            report_html.write('    <div class="w3-container">')
            report_html.write('      <p><b>Modelo de punto de cambio de combustible fósil</b></p>')
            if (hasattr(self.building, 'im_fossil_fuel') and hasattr(self.building.im_fossil_fuel, 'model_description_html')):
                ...
                report_html.write(
                    '<p>No se proporcionan datos de consumo de combustible fósil o no se encontró un modelo de punto de cambio significativo para el combustible fósil.</p>')
            report_html.write('    </div>')
            report_html.write('  </div>    ')
            report_html.write('<div class="w3-half w3-container w3-margin-bottom w3-col m7 w3-padding-small">')
            report_html.write('<div class="w3-container">')
            report_html.write('<p><b>Evaluación comparativa del consumo de combustibles fósiles</b></p>')
            ...
            # Detail EE analysis
            report_html.write('  <div class="w3-container w3-padding-large w3-white">')
            report_html.write('    <h2 id="EE"><b>Detalles de la recomendación de eficiencia energética</b></h2>')
            report_html.write('    <p>Más detalles sobre cada oportunidad de eficiencia energética identificada </p>')
            FIM_des = {
                "Reduce Equipment Schedules": "La carga de sus equipos es más alta que la típica. Los equipos y sistemas dentro de cualquier edificio deben operar usando un horario. Verifique el horario del equipo y si el equipo está operativo durante los tiempos de baja ocupación o durante el uso reducido del edificio. Configure una notificación para identificar cuándo se anulan los horarios y no se vuelven a la normalidad.",
